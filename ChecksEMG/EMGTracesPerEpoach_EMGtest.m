...
%% Align it
% conds={'TM slow','TM mid','TM fast'};
conds={'TM mid','TM fast'};
...
%     condlegend={'Slow_{late}','Mid_{late}','Fast_{late}'};
condlegend={'Mid_{late}','Fast_{late}'};
else
%     condlegend={'Slow_{late}','Mid_{late}','Fast_{late}'};
condlegend={'Mid_{late}','Fast_{late}'};
...
for s=5
    ...
    for m=1:length(muscle)
        ...
        RBaseMid=expData.getAlignedField('procEMGData',conds(1),events,alignmentLengths).getPartialDataAsATS({['R' muscle{m}]});
        LBaseMid=expData.getAlignedField('procEMGData',conds(1),events([3,4,1,2]),alignmentLengths).getPartialDataAsATS({['L' muscle{m}]});
...
%                     A_post=RAdap.getPartialStridesAsATS(find(RAdap.Data(2090:2100)));
                    S=RBaseMid.getPartialStridesAsATS(find(RBaseMid.Data(end-40:end)));%23:73 03 range
...
%                     N=RNeg.getPartialStridesAsATS(find(RNeg.Data(2:10)));
                    ...
                    %                     A_post=RAdap.getPartialStridesAsATS(find(RAdap.Data(2090:2100)));
                    S=RBaseMid.getPartialStridesAsATS(find(RBaseMid.Data(1:30)));%23:73 03 range
                    F=RBaseFast.getPartialStridesAsATS(find(RBaseFast.Data(1:30)));
                    end 
                    tit=['R' muscle{m}];
                case 2
                    if late==1
...
%                     A_post=LAdap.getPartialStridesAsATS(find(RAdap.Data(2090:2100)));
                    S=LBaseMid.getPartialStridesAsATS(find(LBaseMid.Data(end-40:end))); %young 60:95
...
%                     N=LNeg.getPartialStridesAsATS(find(LNeg.Data(2:10)));
                    ...
                    %Early
                    S=LBaseMid.getPartialStridesAsATS(find(LBaseMid.Data(1:30))); %young 60:95
                    F=LBaseFast.getPartialStridesAsATS(find(LBaseFast.Data(1:30)));
                    end 
                    tit=['L' muscle{m}];
